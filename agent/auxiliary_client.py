...
def resolve_provider_client(
    ...
    if pconfig.auth_type == "external_process":
        ...
        if not final_model:
            logger.warning(
                "resolve_provider_client: external-process provider %s requested "
                "but no model was provided or configured",
                provider,
            )
            return None, None
        if not api_key or not base_url or not command:
            logger.warning(
                "resolve_provider_client: external-process provider %s requested "
                "but runtime credentials/command are incomplete",
                provider,
            )
            return None, None
        from agent.copilot_acp_client import CopilotACPClient
